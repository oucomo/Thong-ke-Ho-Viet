...
server <- function(input, output) {
  ...
  output$count1 <- renderValueBox({
    hc12 <- filt_mai1() %>%
      summarise(Tong = round(sum(as.numeric(songuoi)),digits = 0))
  ...
  output$count2 <- renderValueBox({
    hc13 <- filt_mai1() %>%
      summarise(Tong = round(sum(as.numeric(danso)),digits = 0))
  ...
  output$count4 <- renderValueBox({
    hc15 <- filt_mai1() %>%
      summarise(Tong = sum(as.numeric(pro.pop)))
