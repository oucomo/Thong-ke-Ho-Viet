...
server <- function(input, output, session) {
  ...
  filt_mai1 <- reactive({
    ...
    #     if (input$i2_huyen == "Select All") {NAME_2 %in% s2_Huyen} else {NAME_2 == input$i2_huyen}
    # ]
    # attributes(dat3) = attributes(dat)
    # dat3
  })
  ...
  output$map1 <- renderLeaflet({
    if (is.null(filt_mai1())) return()
    rvs$to_load <- isolate(rvs$to_load)+1
    rvs$map <- leaflet(filt_mai1()) %>%
  ...
  observeEvent(c(rvs$to_load,filt_mai1()), {
    # pal1 <- mappalette()
    if (!length(rvs$map)) return()
    ...
    library(future);plan(multicore, workers=10, gc = TRUE)
    m <- vector('list', length(unique(filt_mai1()$NAME_1)))
    t <- 0
    for (tinh in unique(filt_mai1()$NAME_1)[1:5]){
      t = t+1
      print(tinh)
      datinh = filt_mai1() %>% filter(NAME_1 == tinh)
      mappopuupp=force(mappopup())
      m[[t]] <- future(
      leafletProxy("map1", data = datinh) %>%
      addPolygons(stroke = FALSE,
                  smoothFactor = 0.2,
                  fillOpacity = .5,
                  popup = mappopuupp,
                  color = ~ pal(datinh$pro.pop)) %>%
        addLegend("bottomright", pal = pal, values = ~datinh$pro.pop,
