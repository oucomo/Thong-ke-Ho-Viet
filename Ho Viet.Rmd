...
# ## Compute density raster
# # from sf to sp
# nguyen_f <- as(nguyen_f,'Spatial')
# 
# #Using a zero-width buffer cleans up many topology problems in R
# library(rgeos)
# nguyen_f <- gBuffer(nguyen_f, byid=TRUE, width=0)
# 
# # get coodinates
# coords <- coordinates(feat)
# 
# nguyen_coords <- coordinates(nguyen_f)
# # from sp to spatstat
# pts <- ppp(x = coords[,1], y = coords[,2], window = as.owin(feat, 10), 
#            marks = feat$P14_POP)
# 
# nguyen_pts <- ppp(x = nguyen_coords[,1], y = nguyen_coords[,2], window = as.owin(nguyen_f, 10), marks = nguyen_f$pro.pop)
# # compute density
# nguyen_ds <- density.ppp(x = nguyen_pts, sigma = 300, eps = 100, weights = nguyen_pts$marks)
# # spatstat to raster, in inhab per sq. km
# nguyen_ras <- raster(nguyen_ds, crs = proj4string(nguyen_f)) * 1000 * 1000
# nguyen_ras[is.na(nguyen_ras)] <- 0
# 
# ## Compute contour polygons
# # raster break values 
# bks <- c(seq(0,50000, 5000), 54100)
# nguyen_iso_pop <- rasterToContourPoly(r = nguyen_ras, breaks = bks, mask = nguyen_f)
# 
# nguyen_iso_pop <- isopoly(
#   x,
#   nclass = 8,
#   breaks,
#   mask,
...
# iso_pop <- st_as_sf(iso_pop)
# 
# nguyen_iso_pop <- st_as_sf(nguyen_iso_pop)
# # order iso surfaces
# iso_pop <- iso_pop[order(iso_pop$center),]
# 
# nguyen_iso_pop <- nguyen_iso_pop[order(nguyen_iso_pop$center),]
# # color palette à la viridis
# pal <-c("#000004FF", "#170C3AFF", "#420A68FF", "#6B186EFF", "#932667FF", 
#         "#BB3754FF", "#DD513AFF", "#F3771AFF", "#FCA50AFF", "#F6D645FF", 
#         "#FCFFA4FF")
...
# par(mar = c(0,0,1.2,0))
# # plot area
# plot(st_geometry(nguyen_iso_pop), col = NA, border = NA, bg = "ivory1")
# ## tanaka contours + colors
# # plot each contour layer successively
# for(i in 1:nrow(nguyen_iso_pop)){
#   p <- st_geometry(nguyen_iso_pop[i,])
#   # plot light contour polygons with a Nort-West shift
#   plot(p + c(-30, 30), add=T, border = "#ffffff90",col = "#ffffff90")
#   # plot dark contour polygons with a South-East shift
#   plot(p + c(50, -50),  col = "#00000099", add=T, border = "#00000099")
...
#             title.txt = "Inhabitants\nper sq. km *", cex = 0.8)
# # layout
# layoutLayer(title = "Smoothed Population Density, Nguyen 2022",
#             col = "ivory1", tabtitle = T, coltitle = "black",
#             frame = T, scale = 1,
#             sources = "T. Giraud - 2018",
#             author = "Contours...Iris® - IGN 2017, Recensements de la population - Insee 2017")
