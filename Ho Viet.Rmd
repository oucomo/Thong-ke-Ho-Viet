...
poppot <- stewart(knownpts = nguyen_f, 
                  varname = "songuoi", 
                  typefct = "exponential", 
                  span = 1000, 
                  beta = 2, 
...
bv <- quantile(nguyen_f$songuoi, seq(from = 0, to = 1, length.out = 9))
...
# Set a color palette
pal <- carto.pal(pal1 = "yellow.pal", n1 = 8)

# Draw the map
par <- par(mar = c(0,0,1.2,0))

# Draw the basemap
plot(st_geometry(pot), add = F, border = NA, bg = "#cdd2d4")
...
choroLayer(x = pot, var = "center", 
           ...
           legend.values.rnd = -2)
plot(pot, add=T, lwd = 0.5, border = "grey30")
