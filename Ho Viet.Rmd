...
```{r}
la_namdinh <- dat_sff %>% filter(Ho=="Lã" & NAME_1=="Nam Dinh") %>% st_as_sf(crs = 4326)
la_namdinh$centroid <- sf::st_transform(la_namdinh, 2154) %>% 
  st_centroid() %>% 
  # this is the crs from d, which has no EPSG code:
  st_transform(., '+proj=longlat +ellps=GRS80 +no_defs') %>%
  # since you want the centroids in a second geometry col:
...
library(geosphere)
la_namdinh <- la_namdinh %>%
  mutate(centroid_0 = centroid[NAME_3=="Xa Yen Phuong"],
         distm = geosphere::distGeo(st_coordinates(centroid), st_coordinates(centroid_0)),
         distm_km=distm/1000,
         distm_km2=distm_km^2,
         sum_distm_km2=sum(distm_km2),
...
la_namdinh_radius <- la_namdinh %>%
  arrange(distm_km) %>%
  mutate(rank=row_number())

la_namdinh_peak <- la_namdinh %>% filter(NAME_3=="Xa Yen Phuong")
ggsave(filename = file.path("figures", "General", "La-Thai Binh-Density-2(2).jpeg"), width = 25, height = 16)
```

```{r, warning=FALSE, message=FALSE, lazy = TRUE}
ggplot() + geom_sf() + geom_sf(data=la_namdinh, aes(fill=pro.pop)) +
  geom_sf_label(data=la_namdinh_peak, mapping = aes(label=NAME_3), color="red", size=6, stat = "sf_coordinates", hjust=-0.1, vjust=1, label.size = 0.5) +
  ...
  ggtitle("Lã-Nam Định")
ggsave(filename = file.path("figures", "General", "La-Nam Dinh-PerPop.jpeg"), width = 25, height = 16)
```

```{r, lazy = TRUE}
ggplot() + geom_sf() + geom_sf(data=la_namdinh, aes(fill=songuoi_km)) +
  geom_sf_label(data=la_namdinh_peak, mapping = aes(label=NAME_3), color="red", size=6, stat = "sf_coordinates", hjust=-0.1, vjust=1, label.size = 0.5) +
  scale_fill_viridis("Mật độ (người/km2)", direction = -1, option = "viridis") +
  theme_bw() +
  theme(text = element_text(size=45), axis.text.x = element_text(size=30), axis.text.y = element_text(size=30), legend.text=element_text(size=30), legend.key.size = unit(2, "cm"), plot.title = element_text(size = 45)) +
  ggtitle("Lã-Nam Định")
ggsave(filename = file.path("figures", "General", "La-Nam Dinh-PerArea.jpeg"), width = 25, height = 16)
```

## Họ Lã - Ninh Bình

```{r}
la_ninhbinh <- dat_sff %>% filter(Ho=="Lã" & NAME_1=="Ninh Binh") %>% st_as_sf(crs = 4326)
la_ninhbinh$centroid <- sf::st_transform(la_ninhbinh, 2154) %>% 
  st_centroid() %>% 
  # this is the crs from d, which has no EPSG code:
  st_transform(., '+proj=longlat +ellps=GRS80 +no_defs') %>%
  # since you want the centroids in a second geometry col:
...
library(geosphere)
la_ninhbinh <- la_ninhbinh %>%
  mutate(centroid_0 = centroid[NAME_3=="Xa Khanh Thinh"],
         distm = geosphere::distGeo(st_coordinates(centroid), st_coordinates(centroid_0)),
         distm_km=distm/1000,
         distm_km2=distm_km^2,
         sum_distm_km2=sum(distm_km2),
...
la_ninhbinh_radius <- la_ninhbinh %>%
  arrange(distm_km) %>%
  mutate(rank=row_number())

la_ninhbinh_peak <- la_ninhbinh %>% filter(NAME_3=="Xa Khanh Thinh")
```

```{r, warning=FALSE, message=FALSE, lazy = TRUE}
ggplot() + geom_sf() + geom_sf(data=la_ninhbinh, aes(fill=pro.pop)) +
  geom_sf_label(data=la_ninhbinh_peak, mapping = aes(label=NAME_3), color="red", size=6, stat = "sf_coordinates", hjust=-0.1, vjust=1, label.size = 0.5) +
  scale_fill_viridis("Tỉ lệ (%)", direction = -1, option = "viridis") +
  theme_bw() +
  theme(text = element_text(size=45), axis.text.x = element_text(size=30), axis.text.y = element_text(size=30), legend.text=element_text(size=30), legend.key.size = unit(2, "cm"), plot.title = element_text(size = 45)) +
  ggtitle("Lã-Ninh Bình")
ggsave(filename = file.path("figures", "General", "La-Ninh Binh-PerPop.jpeg"), width = 25, height = 16)
```

```{r, lazy = TRUE}
ggplot() + geom_sf() + geom_sf(data=la_ninhbinh, aes(fill=songuoi_km)) +
  geom_sf_label(data=la_ninhbinh_peak, mapping = aes(label=NAME_3), color="red", size=6, stat = "sf_coordinates", hjust=-0.1, vjust=1, label.size = 0.5) +
  scale_fill_viridis("Mật độ (người/km2)", direction = -1, option = "viridis") +
  theme_bw() +
  theme(text = element_text(size=45), axis.text.x = element_text(size=30), axis.text.y = element_text(size=30), legend.text=element_text(size=30), legend.key.size = unit(2, "cm"), plot.title = element_text(size = 45)) +
  ggtitle("Lã-Ninh Bình")
ggsave(filename = file.path("figures", "General", "La-Ninh Binh-PerArea.jpeg"), width = 25, height = 16)
```
...
render_snapshot(filename = "figures/3D2/Phung-PerArea(1).png", clear=T)
# Close the window when you're done
rgl::rgl.close()

plot_gg(gg_phung, multicore = TRUE, raytrace = T, width = 10, height = 15, 
       scale = 500, windowsize = c(3500, 2165), zoom = 0.6, phi = 30, theta = -30)
render_snapshot(filename = "figures/3D2/Phung-PerArea(2).png", clear=T)
```
...
plot_gg(gg_mai, multicore = TRUE, raytrace = T, width = 10, height = 15, 
       scale = 500, windowsize = c(3500, 2165), zoom = 0.6, phi = 30, theta = 30, 
       background = "white", shadow = FALSE, shadowcolor = "white", shadow_darkness=0)
...
gg_ma = ggplot(ma_sff) +
  geom_sf(aes(fill = pro.pop), lwd=0.01) +
  geom_sf(data=river3, col="white") +
  annotate(geom="text", x=104.5, y=16, color="red", size=7.5, label= paste(round(ma_sff$prop.ma[1],1), "% dân số", sep = "")) +
  scale_fill_viridis("Tỉ lệ (%)", direction = -1, option = "viridis") +
  ggtitle("Họ Mã") +
...
#render_movie("figures/3D/ma.mp4",frames = 720, fps=30,zoom=0.6,fov = 30)
#render_snapshot(filename = "figures/3D/Ma.png", clear=T)
render_snapshot(filename = "figures/3D2/Ma-PerPop.png(1)", clear = T)
...
render_snapshot(filename = "figures/3D2/Phung-PerPop(1).png", clear=T)
rgl::rgl.close()

plot_gg(gg_phung, multicore = TRUE, raytrace = T, width = 10, height = 15, 
       scale = 500, windowsize = c(3500, 2165), zoom = 0.6, phi = 30, theta = -30, 
       background = "white", shadow = FALSE)
render_snapshot(filename = "figures/3D2/Phung-PerPop(2).png", clear=T)
rgl::rgl.close()
```
