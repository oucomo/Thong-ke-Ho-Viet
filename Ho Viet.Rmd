...
commune<-mai_nghean_radius$NAME_3
rank<-factor(rank, labels = commune)
estimate<-25*exp(-(distm_km^2)/(2*300))
...
plot_df <- df %>%
  ...
  theme(panel.grid.major = element_blank()) + 
  annotate(geom='text', x=150, y=200, label="y == 25*e^-{(x^{2}+y^{2})/2*(300)}", parse = TRUE,size=10) +
  annotate(geom='text', x=150, y=180, label="s.e. = 44 người/xã",parse=F,size=8) +
  #coord_flip() +
  ggtitle("Mai-Nghệ An")
...
commune<-mai_thanhhoa_radius$NAME_3
rank<-factor(rank, labels = commune)
estimate<-250*exp(-(distm_km^2)/(2*400))
...
plot_df <- df %>%
  ...
  theme(text = element_text(size=20), axis.text.x = element_text(size=5, angle = 90), axis.text.y = element_text(size=20)) +
  theme(panel.grid.major = element_blank()) + 
  annotate(geom='text', x=200, y=500, label="y == 250*e^-{(x^{2}+y^{2})/2*(400)}", parse = TRUE,size=10) +
  annotate(geom='text', x=200, y=450, label="s.e. = 700 người/xã",parse=F,size=8) +
  #coord_flip() +
  ggtitle("Mai-Thanh Hóa")
...
commune<-mai_namdinh_radius$NAME_3
rank<-factor(rank, labels = commune)
estimate<-200*exp(-(distm_km^2)/(2*50))
...
plot_df <- df %>%
  ...
  theme(text = element_text(size=20), axis.text.x = element_text(size=5, angle = 90), axis.text.y = element_text(size=20)) +
  theme(panel.grid.major = element_blank()) + 
  annotate(geom='text', x=50, y=500, label="y == 200*e^-{(x^{2}+y^{2})/2*(50)}", parse = TRUE,size=10) +
  annotate(geom='text', x=50, y=450, label="s.e. = 500 người/xã",parse=F,size=8) +
  #coord_flip() +
  ggtitle("Mai-Nam Định")
