...
+ 
  geom_line(aes(x=reorder(factor(rank), -distm_km), y=estimate))
plot_df
```
...
mai_nghean_dens2 <- mai_nghean_radius %>%
  ...
  geom_line(y =distm_km) +
  #geom_line(aes(x=reorder(NAME_3, -distm_km), y=predict(fit5), col='blue')) +
  stat_summary(aes(label=paste0(round(songuoi_km,0), "", "ng/km2")), geom="text", size=1, position = position_dodge(width= 1), vjust= 0.5, hjust = 0) +
  #stat_smooth(method = 'lm', formula = distm_km ~ poly(reorder(NAME_3, -distm_km),5), aes(colour = 'polynomial'), se= TRUE) +
  #stat_function(fun = dnorm, args = list(mean = 0, sd = 26.2)) +
  #geom_line(aes(x = exp(-distm_km^2/685), y=distm_km)) +
  #stat_smooth(method = 'lm', aes(colour = 'linear'), se = FALSE) +
  ...
  scale_y_continuous(name="Khoảng cách tới tâm (km)") +
  theme_bw() + 
  theme(text = element_text(size=20), axis.text.x = element_text(size=20), axis.text.y = element_text(size=3)) +
  theme(panel.grid.major = element_blank()) + 
  annotate(geom='text', x=300, y=75, label="y == 400*e^-{(x^{2}+y^{2})/2*(52.3)}", parse = TRUE,size=10) +
  coord_flip() +
  ggtitle("Mai-Nghệ An")
