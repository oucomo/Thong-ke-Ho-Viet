...
plot_df <- df %>%
  ...
  ggplot(aes(x=reorder(factor(rank), distm_km), y=songuoi_km)) +
  ...
  scale_x_discrete(name = "Xã/Phường theo khoảng cách tới tâm (km)", 
...
plot_df
ggsave(filename = file.path("figures", "General", "Tran-Nam Dinh-Density-2(2).jpeg"), width = 25, height = 16)
```

```{r}
tran_namdinh_radius50 <- tran_namdinh %>%
  arrange(desc(distm_km))

tran_namdinh_radius50 <- tran_namdinh_radius50 %>% slice(178:227)

tran_namdinh_dens2 <- tran_namdinh_radius50 %>%
  ggplot(aes(x = reorder(NAME_3, distm_km, decreasing=T), y =distm_km)) +
  geom_bar(stat = "identity", width = 0.75) +
  stat_summary(aes(label=paste0(round(songuoi_km,0), "ng/km2")), geom="text", size=2, position = position_dodge(width= 1), vjust= 0.5, hjust = 0) +
  scale_x_discrete(name = "Xã/Phường") +
  scale_y_continuous(name="Khoảng cách tới tâm (km)") +
  ...
  theme(axis.text.y = element_text(size = 7),
        panel.grid.major = element_blank()) + coord_flip() +
  ggtitle("Trần-Nam Định")
ggsave(filename = file.path("figures", "General", "Tran-Nam Dinh-Density(2).jpeg"), width = 10, height = 7)
```
...
plot_df <- df %>%
  ...
  scale_y_continuous(name="Mật độ người/km2") +
  theme_bw() + 
  theme(text = element_text(size=20), axis.text.x = element_text(size=6, angle = 90), axis.text.y = element_text(size=20)) +
  theme(panel.grid.major = element_blank()) + 
  #annotate(geom='text', x=30, y=700, label="y == 200*e^-{(x^{2}+y^{2})/2*(100)}", parse = TRUE,size=10) +
  #annotate(geom='text', x=30, y=650, label="s.e. = 600 người/xã",parse=F,size=8) +
  #coord_flip() +
  ggtitle("Mai-Thanh Hóa")
...
plot_df <- df %>%
  ...
  theme(panel.grid.major = element_blank()) + 
  #annotate(geom='text', x=30, y=500, label="y == 220*e^-{(x^{2}+y^{2})/2*(40)}", parse = TRUE,size=10) +
  #annotate(geom='text', x=30, y=450, label="s.e. = 500 người/xã",parse=F,size=8) +
  #coord_flip() +
  ggtitle("Mai-Nam Định")
...
plot_df <- df %>%
  ...
  scale_y_continuous(name="Mật độ người/km2") +
  theme_bw() + 
  theme(text = element_text(size=20), axis.text.x = element_text(size=15, angle = 90), axis.text.y = element_text(size=20)) +
  theme(panel.grid.major = element_blank()) + 
  #annotate(geom='text', x=20, y=80, label="y == 50*e^-{(x^{2}+y^{2})/2*(36)}", parse = TRUE,size=10) +
  #annotate(geom='text', x=20, y=70, label="s.e. = 50 người/xã",parse=F,size=8) +
  #coord_flip() +
  ggtitle("Khúc-Hưng Yên")
