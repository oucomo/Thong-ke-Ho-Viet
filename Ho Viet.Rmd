...
#data
nguyen_sf <- dat_sf %>% mutate(prop.nguyen=(sum(subset(., Ho=="Nguyễn")[, "songuoi"])/sum(songuoi))*100) %>% filter(Ho=="Nguyễn")
#nguyen_f <- nguyen_sf %>% select(MAXA, MATINH, songuoi, pro.pop, geometry) %>% filter(MATINH=="01") %>% st_as_sf(crs = 4326)
...
nguyen_f_stewart <- stewart(knownpts = nguyen_f, varname = "songuoi", typefct = "exponential", span = 1000, beta = 3, mask = nguyen_f, returnclass = "sf")

# Create contour
contourpoly <- isopoly(x = nguyen_f_stewart, nclass = 6, mask = nguyen_f, returnclass = "sf")

library(sf)
png(filename = "Nguyen_contour.png", width = 800, height = 500, res = 100, bg = NA)
...
lwd = 0.2,
legend.title.txt = "Số người ước tính\ncủa dòng họ (Nguyễn)\ntại khu vực lân cận",
legend.values.rnd = 0)
plot(st_geometry(nguyen_f), add = TRUE)
propSymbolsLayer(x = nguyen_f, var = "songuoi",
legend.pos = "right",
legend.title.txt = "Số người",
col = "#ff000020")
layoutLayer(title = "Phân bố số người ước tính của dòng họ (Nguyễn)",
sources = "", author = "")
par(opar)
}
dev.off()
#fwrite(x = nguyen_f_stewart, file = file.path("nguyen_f_stewart.csv"))
```
## Họ Lý
...
# Compute Stewart potentials
ly_f_stewart <- stewart(knownpts = ly_f, varname = "songuoi", typefct = "exponential", span = 1000, beta = 3, mask = ly_f, returnclass = "sf")

# Create contour
contourpoly <- isopoly(x = ly_f_stewart, nclass = 6, mask = ly_f, returnclass = "sf")

library(sf)
png(filename = "Ly_contour.png", width = 800, height = 500, res = 100, bg = NA)
plot(st_geometry(contourpoly))
...
legend.title.txt = "Số người ước tính\ncủa dòng họ (Lý)\ntại khu vực lân cận",
legend.values.rnd = 0)
plot(st_geometry(ly_f), add = TRUE)
propSymbolsLayer(x = ly_f, var = "songuoi",
legend.pos = "right",
legend.title.txt = "Số người",
col = "#ff000020")
layoutLayer(title = "Phân bố số người ước tính của dòng họ (Lý)",
sources = "", author = "")
par(opar)
}
dev.off()
...
# Draw the basemap
png(filename = "Nguyen_contourpoly.jpeg", width = 1000, height = 2000, res = 100, bg = NA)
...
choroLayer(x = pot_rot, var = "center", 
          ...
          legend.title.txt = "Tỉ lệ(%)\ntại khu vực lân cận",
          legend.values.rnd = 1)
# Set a layout
layoutLayer(title = "Phân bố tỉ lệ (%) của dòng họ (Nguyễn)",
sources = "Vietnam GSO", author = "Duc Du, 2022",north = T, theme = "wine.pal")
par(opar)
}
dev.off()
```
...
library(potential)
library(mapsf)
mf_map(nguyen_f, col= "grey80", border = "white", lwd = .4)
y <- create_grid(x = nguyen_f, res = 100000)
mf_map(y, pch = 23, add = TRUE, bg = "blue", cex = .5 )

d <- create_matrix(x = nguyen_f, y = y)

y$pot <- potential(x = nguyen_f, y = y, d = d,
                   var = "songuoi", fun = "e",
                   span = 75000, beta = 2)
mf_map(nguyen_f, col= "grey80", border = "white", lwd = .4)
...
y <- create_grid(x = nguyen_f, res = 20000)
y$pot <- mcpotential(x = nguyen_f, y = y, 
                     var = "songuoi", fun = "e",
                     span = 75000, beta = 2, 
                     limit = 200000, ncl = 2)
y$pot2 <- 100 * y$pot / max(y$pot)
...
mf_map(x = iso, var = "min", type = "choro", 
       ...
       leg_val_rnd = 0,
       leg_title = "Mật độ ước tính")
mf_credits(txt = "© Địa giới hành chính theo bản đồ Việt Nam and © Dữ liệu dân số từ TCTK Việt Nam",
           pos = "bottomright", cex = .7)
mf_title(txt = "Họ Nguyễn", bg = "white", fg = "black", inner = TRUE, cex = 1.1)
```
