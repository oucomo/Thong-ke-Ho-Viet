...
```{r, message=FALSE, warning=FALSE}
bui_sf <- dat_sf %>% mutate(prop.bui=(sum(subset(., Ho=="Bùi")[, "songuoi"])/sum(songuoi))*100) %>% filter(Ho=="Bùi")
bui_f <- bui_sf %>% select(MAXA, MATINH, songuoi, pro.pop, geometry) %>% st_as_sf(crs = 4326)
...
ho_sf2 <- sf::st_transform(pham_sf2, 2154)
...
choroLayer(x = pot_rot, var = "center", 
          ...
          legend.values.rnd = 1)
# Set a layout
layoutLayer(title = paste("Đường đồng mức tỉ lệ (%) trên dân số họ Khúc", paste(round(khuc_plot$prop.khuc[1],1), "% dân số", sep = ""), sep=" - "),
sources = "Data source: Vietnam GSO", author = "Duc Du, 2022", theme = "wine.pal")
par(par)
}
dev.off()
...
choroLayer(x = pot_rot, var = "center", 
          ...
          legend.values.rnd = 1)
# Set a layout
layoutLayer(title = paste("Đường đồng mức tỉ lệ (%) trên dân số họ Tôn", paste(round(ton_plot$prop.ton[1],1), "% dân số", sep = ""), sep=" - "),
sources = "Data source: Vietnam GSO", author = "Duc Du, 2022", theme = "wine.pal")
par(par)
}
dev.off()
