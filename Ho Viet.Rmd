...
```{r}
tran_sf <- dat_sf %>% filter(Ho=="Trần")
tran_plot <- tran_sf %>% select(pro.pop, geometry) %>% st_as_sf(crs = 4326)
...
```{r}
le_sf <- dat_sf %>% filter(Ho=="Lê")
le_plot <- le_sf %>% select(pro.pop, geometry) %>% st_as_sf(crs = 4326)
...
```{r}
du_sf <- dat_sf %>% filter(Ho=="Dư")
du_plot <- du_sf %>% select(pro.pop, geometry) %>% st_as_sf(crs = 4326)
...
```{r}
trieu_sf <- dat_sf %>% filter(Ho=="Triệu")
trieu_plot <- trieu_sf %>% select(pro.pop, geometry) %>% st_as_sf(crs = 4326)
